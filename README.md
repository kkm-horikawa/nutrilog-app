...
### Phase 2: P2Pデータ共有（サーバ不要・完全無料維持）

**サーバを立てない。** LINE・メール等で直接データを共有する設計。
...
|------|-----------|-------------------|
| インフラ代 | 月額発生 | **0円（永久無料）** |
| 永続性 | サーバ止めたら終わり | **端末にある限り永続** |
| プライバシー | サーバにデータ保存 | **自分の端末のみ** |
| データ共有 | 自動だが匿名 | **友達と直接、信頼性高い** |
| 運営負担 | 承認作業が必要 | **ユーザー同士で完結** |
| 差別化 | 他アプリと同じ | **新しいアプローチ** |
...
### 友達からの共有データ（Phase 2: P2P共有）

- 文科省DBにない食品（ポテチ、カップ麺、チェーン店メニュー等）
- 友達がLINE等で送ってくれたカスタム食品をインポート
- サーバ不要、完全ローカルで完結
...
│  💬 LINE                                        │
│                                                 │
│  [自分]: NutriLogの食品データ送るね！           │
│                                                 │
│  📎 nutrilog_foods_20250116.json               │
│     3件の食品データ                             │
│                                                 │
└─────────────────────────────────────────────────┘
...
│                                                 │
│  送信者: 友達A                                  │
│  ファイル: nutrilog_foods_20250116.json         │
│                                                 │
│  含まれる食品 (3件):                            │
│  ├ カルビー ポテトチップス うすしお 60g         │
│  │   336kcal / P:2.8g / F:21g / C:33g          │
│  ├ 日清 カップヌードル                          │
│  │   351kcal / P:10.5g / F:14.6g / C:44.5g     │
│  └ セブン サラダチキン                          │
│      105kcal / P:23.8g / F:0.9g / C:0.3g       │
│                                                 │
│  ⚠️ インポート前に内容を確認してください         │
│                                                 │
│  [キャンセル]              [インポート]         │
└─────────────────────────────────────────────────┘
```
...
| 観点 | サーバ型（従来） | **P2P共有（NutriLog）** |
|------|----------------|----------------------|
| 信頼性 | 匿名ユーザーの投稿 | **友達からのデータ** |
| 精度 | バラつきあり | **送り主に確認可能** |
| 即時性 | 承認待ち | **受け取り即使える** |
| 運営 | 管理者が必要 | **完全自律** |
...
```json
{
  ...
  "foods": [
    {
      "name": "カルビー ポテトチップス うすしお",
      "barcode": "4901330573287",
      "serving_size": 60,
      "serving_unit": "g",
      "nutrients": {
        "calories": 336,
...
- [ ] 栄養成分表OCR（Google ML Kit）
- [ ] P2P食品データ共有（LINE・メール・ファイル）
- [ ] 食品データのインポート/エクスポート（JSON）
- [ ] 週間・月間レポート
- [ ] エクスポート機能（CSV）
...
|------|------|
| 対象 | 全機能 or プレミアム機能のみ |
| メリット | サブスク疲れのユーザーに刺さる |
| 価格帯 | 100〜300円（缶コーヒー1〜2本分） |
